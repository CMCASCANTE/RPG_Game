...
class MenuBase(ctk.CTkFrame):
    ...
    def __init__(self, master, **kwargs):
        # master será la ventana principal que viene de Menu.py
        super().__init__(master, fg_color="transparent", **kwargs)
        self.pack(fill="both", expand=True)

    def limpiar_interfaz(self):
        # Elimina todos los widgets del container para cargar una nueva interfaz
        for widget in self.container.winfo_children():
            widget.destroy()

    def mostrar_menu_principal(self):
        self.limpiar_interfaz()  # Limpiamos antes de dibujar el menú

        # --- Fondo del menú principal ---
        if self.imagen_fondo_ctk:
            self.label_fondo = ctk.CTkLabel(
                self.container, image=self.imagen_fondo_ctk, text=""
            )
        ...
        # --- Botón NUEVA PARTIDA (calibrado para tu imagen) ---
        self.btn_jugar = ctk.CTkButton(
            self.label_fondo,
            text="NUEVA PARTIDA",
            command=self.mostrar_interfaz_juego,  # <-- ¡Aquí está el cambio!
            fg_color="#2c3e50",
            hover_color="#34495e",
            font=("Fixedsys", 18),
            width=220,
        ...
        self.btn_jugar.place(relx=0.5, rely=0.55, anchor="center")

        # --- Botón SALIR (calibrado para tu imagen) ---
        ...
        self.btn_salir.place(relx=0.5, rely=0.72, anchor="center")

    def mostrar_interfaz_juego(self):
        self.limpiar_interfaz()  # Limpiamos la interfaz del menú principal

        # --- Fondo para la interfaz del juego (el mismo fondo por ahora) ---
        if self.imagen_fondo_ctk:
            self.label_fondo_juego = ctk.CTkLabel(
                self.container, image=self.imagen_fondo_ctk, text=""
            )
            self.label_fondo_juego.place(x=0, y=0, relwidth=1, relheight=1)
